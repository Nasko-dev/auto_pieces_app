...
class ImmatriculationService {
  ...
  Either<Failure, VehicleInfo> _parseResponse(
      String responseBody, String plate) {
    try {
  ...
  VehicleInfo _extractVehicleInfo(
      Map<String, dynamic> vehicleData, String plate) {
    final vehicleInfo =
        vehicleData['vehicleInformation'] as Map<String, dynamic>? ?? {};
    ...
    final engine = engines.isNotEmpty
        ? engines.first as Map<String, dynamic>
        : <String, dynamic>{};
    final environmental =
        engine['environmental'] as Map<String, dynamic>? ?? {};

    // Extraire le nom du modèle proprement
    // fullModel peut contenir : "PEUGEOT 206+ (2L_, 2M_) 1.4 HDi eco 70"
    // On veut juste : "206+"
    String? modelName =
        vehicleInfo['model']?.toString(); // Ex: "206+ (2L_, 2M_)"
    ...
    return VehicleInfo(
      ...
      year: _extractYearFromDate(
              vehicleInfo['firstRegistrationDate']?.toString()) ??
  ...
  String _buildDescription(
      Map<String, dynamic> vehicleInfo, Map<String, dynamic> engine) {
    final parts = <String>[];
    ...
    final power =
        engine['powerHP']?.toString() ?? engine['powerKW']?.toString();
  ...
  Future<Either<Failure, int>> checkRemainingCredits() async {
    ...
    try {
      ...
      return Left(
          NetworkFailure('Erreur lors de la vérification des crédits: $e'));
    }
  }
